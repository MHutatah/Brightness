...
class MyApplication(QtWidgets.QMainWindow):
    ...
    def generate_brightness_sources(self):
        ...
        if self.no_of_connected_dev < 2:
            ...
            self.ui.FocusViewBox.setEnabled(False)
            return
    ...
    def connect_handlers(self):
        ...
        if self.ui.FocusViewBox.isEnabled() :
            self.connect_FocusView()
    ...
    def connect_FocusView(self):
        self.ui.FocusDimPr.valueChanged[int].connect(self.activate_focus_view)
        # ActiveWindowID = Executor.execute_command("xdotool getactivewindow")
        # while (self.ui.FocusViewBox.isEnabled()):
        #     if ActiveWindowID != Executor.execute_command("xdotool getactivewindow"):
        #                 self.activate_focus_view()
        #     ActiveWindowID = Executor.execute_command("xdotool getactivewindow")
    ...
    def activate_focus_view(self):
        "Focsued View Activation"

        cmd_value = "xwininfo -id $(xdotool getactivewindow) | grep \"Absolute upper-left X:  \""
        WindowXcordinates = Executor.execute_command(cmd_value).decode('ascii')
        if len(WindowXcordinates) < 31 or (int(WindowXcordinates[26:30])<1920):
            cmd_value2 = "xrandr --output %s --brightness %s" % (self.display2, self.values[self.ui.FocusDimPr.value()])
            Executor.execute_command(cmd_value2)
            cmd_value2 = "xrandr --output %s --brightness %s" % (self.display1, 1)
            Executor.execute_command(cmd_value2)
        else :
            cmd_value2 = "xrandr --output %s --brightness %s" % (self.display1, self.values[self.ui.FocusDimPr.value()])
            Executor.execute_command(cmd_value2)
            cmd_value2 = "xrandr --output %s --brightness %s" % (self.display2, 1)
            Executor.execute_command(cmd_value2)
    ...
    def FocusView_Value(self, focus_br):
        self.ui.FocusDimPr.setValue(focus_br)
